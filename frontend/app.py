...
if page == "Dataset Upload":
    ...
    .card-success { border-left-color: #00A86B; }
    .card-danger { border-left-color: #DC143C; }
    .card-warning { border-left-color: #FF6B35; }
    .card-info { border-left-color: #20B2AA; }
    .card-purple { border-left-color: #9370DB; }
    </style>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        HOSPITAL_COLORS = {
            ...
            'gradient_risk': [[0, '#00A86B'], [0.5, '#FF6B35'], [1, '#DC143C']],  # Risk gradient
        ...
        # Row 1: Readmission Overview Cards
        st.markdown("### 🎯 Readmission Overview")
        col1, col2 = st.columns(2)
        
        with col1:
            # CARD 1: Overall Readmission Distribution
            with st.container():
                st.markdown('<div class="card-title">📊 Overall Readmission Distribution</div>', unsafe_allow_html=True)
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_readmission_dist")
        
        with col2:
            # CARD 2: Readmission Rate Gauge
            with st.container():
                st.markdown('<div class="card-title">🎯 Readmission Rate Gauge</div>', unsafe_allow_html=True)
                fig = go.Figure(go.Indicator(
                    mode="gauge+number+delta",
                    value=readmission_rate * 100,
                    domain={'x': [0, 1], 'y': [0, 1]},
                    title={'text': "Readmission Rate (%)", 'font': {'size': 18}},
                ...
                fig.update_layout(height=380, font={'size': 14}, margin=dict(t=20, b=20, l=20, r=20))
        ...
        with col1:
            # CARD 3: Readmission by Diagnosis
            with st.container():
                st.markdown('<div class="card-title">🔬 Readmission Rate by Primary Diagnosis</div>', unsafe_allow_html=True)
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_diagnosis")
        
        with col2:
            # CARD 4: Treatment Type Distribution
            with st.container():
                st.markdown('<div class="card-title">💊 Treatment Type Distribution by Outcome</div>', unsafe_allow_html=True)
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_treatment")
        
        st.markdown("<br>", unsafe_allow_html=True)
        
        # Row 3: Demographics & Stay Duration Cards
        st.markdown("### 📈 Patient Demographics & Stay Duration")
        col1, col2 = st.columns(2)
        
        with col1:
            # CARD 5: Age Distribution
            with st.container():
                st.markdown('<div class="card-title">👥 Age Distribution by Readmission Status</div>', unsafe_allow_html=True)
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_age")
        
        with col2:
            # CARD 6: Length of Stay Distribution
            with st.container():
                st.markdown('<div class="card-title">⏱️ Length of Stay Distribution by Outcome</div>', unsafe_allow_html=True)
                df_plot = df.copy()
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_los")
        ...
        with col1:
            # CARD 9: Insurance Type Distribution
            with st.container():
                st.markdown('<div class="card-title">🏥 Insurance Type Distribution</div>', unsafe_allow_html=True)
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_insurance")
        
        with col2:
            # CARD 10: Previous Readmissions Impact
            with st.container():
                st.markdown('<div class="card-title">🔄 Impact of Previous Readmissions</div>', unsafe_allow_html=True)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate='<b>Previous: %{x}</b><br>Rate: %{y:.1%}<extra></extra>'
                ))
                ...
                fig.add_trace(go.Bar(
                    ...
                    opacity=0.6,
                    yaxis='y2',
                    hovertemplate='<b>Previous: %{x}</b><br>Patients: %{y}<extra></extra>'
                ))
                ...
                st.plotly_chart(fig, use_container_width=True, key="card_prev_readm")
        
        st.markdown("<br>", unsafe_allow_html=True)
        
        # Row 6: Risk Factors Analysis Cards
        st.markdown("### ⚠️ Risk Factors Analysis")
        col1, col2 = st.columns(2)
        
        with col1:
            # CARD 11: Comorbidities vs Medications Scatter
            with st.container():
                st.markdown('<div class="card-title">🔬 Comorbidities vs Medications (Risk Analysis)</div>', unsafe_allow_html=True)
                ...
                if len(df_sample) > 0:
                    ...
                    df_sample['risk_size'] = df_sample['readmission_risk_score'] * 50 + 5
                    ...
                    st.plotly_chart(fig, use_container_width=True, key="card_scatter")
                else:
                    st.warning("⚠️ Not enough valid data for scatter plot visualization.")
        
        with col2:
            # CARD 12: Correlation Heatmap
            with st.container():
                st.markdown('<div class="card-title">📊 Risk Factor Correlation Heatmap</div>', unsafe_allow_html=True)
                ...
                if len(numeric_cols) > 1:
                    ...
                    if len(df_corr) > 0:
                        ...
                        fig.update_xaxes(tickangle=-45)
                        st.plotly_chart(fig, use_container_width=True, key="card_heatmap")
                    else:
