...
def _render_results(response: dict) -> None:
    ...
    if admit == "Admit":
        st.markdown(
            ...
            '<span style="font-size:1.4rem;font-weight:700;color:#dc2626;">🔴 ADMIT</span>'
            '</div>',
        ...
        st.markdown(
            ...
            'border-radius:8px;margin-bottom:12px;">'
            '<span style="font-size:1.4rem;font-weight:700;color:#16a34a;">🟢 DO NOT ADMIT</span>'
            '</div>',
            unsafe_allow_html=True,
        )
    else:
...
def render_llm_advisor_page(api_url: str) -> None:
    """Main entry point — renders the full LLM Medical Advisor page."""
    _init_session_state()

    # ── Header ───────────────────────────────────────────────────────────────
    st.markdown(
        '<h1 style="color:#008080;">🤖 LLM Medical Advisor</h1>',
        unsafe_allow_html=True,
    )
    st.markdown(
        "Provide clinical notes and optionally upload patient history and lab results. "
        "The AI advisor will generate a structured admission recommendation."
    )
